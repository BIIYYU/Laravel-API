            ...
            <table class="table table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $dt['judul'] }}</td>
                        <td>{{ $dt['pengarang'] }}</td>
                        <td>{{ date('d/m/Y',strtotime($dt['tanggal_publikasi'])) }}</td>
                        <td>
                            <a href="{{ url('buku/' . $dt['id']) }}" class="btn btn-info btn-sm">Edit</a>
                            <form action="{{ url('buku/'.$dt['id']) }}" method="post" onsubmit="return confirm('Apakah yakin akan menghapus data ini')" class="d-inline">
                                @csrf
                                @method('delete')
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    @foreach ($data['links'] as $link)
                        <li class="page-item {{ $link['active']?'active':'' }}"><a class="page-link" href="{{ $link['url2'] }}">{!! $link['label'] !!}</a></li>
                    @endforeach
                </ul>
            </nav>
            @endif
